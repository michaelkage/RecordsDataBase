...
public sealed class AuthenticationService
{
    ...
    public async Task InitializeAsync()
    {
        ...
        data.Users.Add(new UserAccount { Username = "admin", Role = "Admin", PasswordHash = hash, PasswordSalt = salt, MustChangePassword = true });
        await _dataService.SaveAsync(data);
    }

    public async Task<UserAccount?> AuthenticateAdminAsync(string username, string password) => await AuthenticateAsync(username, password, "Admin");
    public async Task<UserAccount?> AuthenticateStudentAsync(string studentId, string password) => await AuthenticateAsync(studentId, password, "Student");
    ...
    public async Task ChangePasswordAsync(string username, string currentPassword, string newPassword)
    {
        ...
        if (account is null || !PasswordHasher.Verify(currentPassword, account.PasswordHash, account.PasswordSalt)) throw new InvalidOperationException("The current password is incorrect.");
    ...
    public async Task EnsureStudentAccountAsync(string studentId, string initialPassword)
    {
        ...
        data.Users.Add(new UserAccount { Username = studentId, StudentId = studentId, Role = "Student", PasswordHash = hash, PasswordSalt = salt, MustChangePassword = true });
        await _dataService.SaveAsync(data);
    }
    ...
    private async Task<UserAccount?> AuthenticateAsync(string username, string password, string role)
    {
        ...
        return account is not null && PasswordHasher.Verify(password, account.PasswordHash, account.PasswordSalt) ? account : null;
    }
}
