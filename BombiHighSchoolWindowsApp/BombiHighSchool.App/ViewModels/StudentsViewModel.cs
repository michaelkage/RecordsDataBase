...
public partial class StudentsViewModel : ObservableObject
{
    ...
    private async Task SaveAsync()
    {
        ...
        try
        {
            ...
            {
                student = await _studentService.AddAsync(Name, age, Gender, ClassLevel);
            ...
            details.Arm = Arm.Trim(); details.Department = Department.Trim(); details.DateOfBirth = DateOfBirth.Trim(); details.AdmissionNumber = AdmissionNumber.Trim();
            details.ParentName = ParentName.Trim(); details.ParentPhone = ParentPhone.Trim(); details.Address = Address.Trim(); details.Email = Email.Trim(); details.Status = Status;
    ...
    private async Task EditAsync(Student? student)
    {
        ...
        Arm = details?.Arm ?? ""; Department = details?.Department ?? ""; DateOfBirth = details?.DateOfBirth ?? ""; AdmissionNumber = details?.AdmissionNumber ?? "";
        ParentName = details?.ParentName ?? ""; ParentPhone = details?.ParentPhone ?? ""; Address = details?.Address ?? ""; Email = details?.Email ?? ""; Status = details?.Status ?? "Active";
    ...
    private async Task DeleteAsync(Student? student)
    {
        if (student is null) return;
        IsBusy = true;
        try { await _studentService.DeleteAsync(student.Id); var data = await _dataService.LoadAsync(); data.StudentDetails.RemoveAll(d => d.StudentId == student.Id); data.Scores.RemoveAll(s => s.StudentId == student.Id); await _dataService.SaveAsync(data); StatusMessage = $"{student.Name} deleted."; await ReloadWithoutBusyMessageAsync(); ClearForm(); }
    ...
    private void ApplyFilter()
    {
        ...
        var filtered = string.IsNullOrWhiteSpace(q) ? _allStudents : _allStudents.Where(s => s.Id.Contains(q, StringComparison.OrdinalIgnoreCase) || s.Name.Contains(q, StringComparison.OrdinalIgnoreCase) || s.ClassLevel.Contains(q, StringComparison.OrdinalIgnoreCase) || s.Gender.Contains(q, StringComparison.OrdinalIgnoreCase)).ToList();
        Students = new ObservableCollection<Student>(filtered);
    }
    private void ClearForm()
    {
        SelectedStudent = null; Name = ""; AgeText = ""; Gender = ""; ClassLevel = ""; Arm = ""; Department = ""; DateOfBirth = ""; AdmissionNumber = ""; ParentName = ""; ParentPhone = ""; Address = ""; Email = ""; Status = "Active"; IsEditing = false;
    }
}
